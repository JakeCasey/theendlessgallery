/******/ (function(modules) {
  ...
  /******/ [
      ...
      AFRAME.registerComponent('motion-capture-recorder', {
        ...
        recordEvent: function(evt) {
          ...
          if (
            'detail' in evt &&
            'state' in evt.detail &&
            typeof evt.detail.state === 'object' &&
        ...
        getJSONData: function() {
          var data;
          var trackedControlsComponent = this.el.components[
            'tracked-controls-webxr'
          ];
          var controller =
            trackedControlsComponent && trackedControlsComponent.controller;
        ...
        saveCapture: function(binary) {
          ...
          var fileName =
            'motion-capture-' + document.title + '-' + Date.now() + '.json';
        ...
        tick: (function() {
          ...
          return function(time, delta) {
            ...
            this.el.components.stroke.drawPoint(
              position,
              rotation,
              time,
              pointerPosition,
      ...
      AFRAME.registerComponent('motion-capture-replayer', {
        ...
        updateRecorder: function(newRecorderEl, oldRecorderEl) {
          if (oldRecorderEl && oldRecorderEl !== newRecorderEl) {
            oldRecorderEl.removeEventListener(
              'strokestarted',
        ...
        updateSrc: function(src) {
          ...
          ].loadRecordingFromUrl(src, false, this.startReplaying.bind(this));
        },
        ...
        startReplaying: function(data) {
          ...
          if (data.gamepad) {
            ...
            el.sceneEl.systems['motion-capture-replayer'].gamepads.push(
              data.gamepad,
            );
        ...
        playRecording: function(delta) {
          ...
          while (
            (currentPose && this.currentPoseTime >= currentPose.timestamp) ||
            ...
            if (
              currentEvent &&
              this.currentPoseTime >= currentEvent.timestamp
            ) {
              if (
                this.currentEventIndex === playingEvents.length &&
                this.data.loop
              ) {
                this.currentEventIndex = 0;
    ...
    /***/ function(module, exports, __webpack_require__) {
      ...
      var log = AFRAME.utils.debug(
        'aframe-motion-capture:avatar-recorder:info',
      );
      var warn = AFRAME.utils.debug(
        'aframe-motion-capture:avatar-recorder:warn',
      );
      ...
      AFRAME.registerComponent('avatar-recorder', {
        ...
        throttledTick: function() {
          ...
          trackedControllerEls.forEach(function setupController(
            ...
            self.trackedControllerEls[
              trackedControllerEl.id
            ] = trackedControllerEl;
            if (self.isRecording) {
        ...
        getJSONData: function() {
          ...
          // Camera.
          data.camera = this.cameraEl.components[
            'motion-capture-recorder'
          ].getJSONData();

          // Tracked controls.
          Object.keys(trackedControllerEls).forEach(function getControllerData(
            id,
          ) {
            data[id] = trackedControllerEls[id].components[
              'motion-capture-recorder'
            ].getJSONData();
          });
    ...
    /***/ function(module, exports, __webpack_require__) {
      ...
      var error = AFRAME.utils.debug(
        'aframe-motion-capture:avatar-replayer:error',
      );
      var log = AFRAME.utils.debug(
        'aframe-motion-capture:avatar-replayer:info',
      );
      var warn = AFRAME.utils.debug(
        'aframe-motion-capture:avatar-replayer:warn',
      );
      ...
      AFRAME.registerComponent('avatar-replayer', {
        ...
        update: function(oldData) {
          ...
          if (
            oldData.spectatorMode !== data.spectatorMode ||
            spectatorModeUrlParam
          ) {
        ...
        activateSpectatorCamera: function() {
          ...
          if (!spectatorCameraEl.hasLoaded) {
            spectatorCameraEl.addEventListener(
              'loaded',
              bind(this.activateSpectatorCamera, this),
            );
            return;
        ...
        initSpectatorCamera: function() {
          ...
          if (this.el.querySelector('#spectatorCameraRig')) {
            this.spectatorCameraEl = sceneEl.querySelector(
              '#spectatorCameraRig',
            );
            return;
          }
          ...
          spectatorCameraRigEl =
            sceneEl.querySelector('#spectatorCameraRig') ||
          ...
          spectatorCameraEl =
            sceneEl.querySelector('#spectatorCamera') ||
            document.createElement('a-entity');
        ...
        replayRecordingFromSource: function() {
          ...
          recordingdb.getRecordingNames().then(function(recordingNames) {
            ...
            if (recordingNames.indexOf(queryParamSrc) !== -1) {
              ...
              recordingdb
                .getRecording(queryParamSrc)
                .then(bind(self.startReplaying, self));
              return;
            }
            ...
            if (src) {
              ...
              self.loadRecordingFromUrl(
                src,
                false,
                bind(self.startReplaying, self),
              );
              return;
            }
            ...
            if (recordingNames.indexOf(self.data.recordingName) !== -1) {
              log(
                'Replaying `' + self.data.recordingName + '` from IndexedDB.',
              );
              recordingdb
                .getRecording(self.data.recordingName)
                .then(bind(self.startReplaying, self));
            }
          });
        },
        ...
        startReplaying: function(replayData) {
          ...
          Object.keys(replayData).forEach(function setReplayer(key) {
            ...
            if (key === 'camera') {
              ...
              //disable look controls on camera while replaying
            } else {
              // Grab other entities.
            ...
            // Force element to be visible - JC
        ...
        configureHeadGeometry: function() {
          ...
          if (
            cameraEl.getObject3D('mesh') ||
            cameraEl.getObject3D('replayerMesh')
          ) {
            return;
          }
          ...
          leftEyeMesh.material = new THREE.MeshBasicMaterial({
            color: 'white',
          });
          ...
          leftEyeBallMesh.material = new THREE.MeshBasicMaterial({
            color: 'black',
          });
          ...
          rightEyeMesh.material = new THREE.MeshBasicMaterial({
            color: 'white',
          });
          ...
          rightEyeBallMesh.material = new THREE.MeshBasicMaterial({
            color: 'black',
          });
      ...
      AFRAME.registerComponent('stroke', {
        ...
        init: function() {
          ...
          this.geometry.addAttribute(
            'position',
            new THREE.BufferAttribute(this.vertices, 3).setDynamic(true),
          );
          this.geometry.addAttribute(
            'uv',
            new THREE.BufferAttribute(this.uvs, 2).setDynamic(true),
          );
          this.geometry.addAttribute(
            'normal',
            new THREE.BufferAttribute(this.normals, 3).setDynamic(true),
          );
        ...
        computeVertexNormals: function() {
          ...
          */
          ...
          this.normals[this.idx - 2 * 3 + 1] =
            this.normals[this.idx - 2 * 3 + 1] / 2;
          this.normals[this.idx - 2 * 3 + 2] =
            this.normals[this.idx - 2 * 3 + 2] / 2;
    ...
    /***/ function(module, exports) {
      AFRAME.registerSystem('motion-capture-replayer', {
        init: function() {
          ...
          trackedControlsTick =
            AFRAME.components['tracked-controls-webxr'].Component.prototype
              .tick;
          ...
          trackedControlsSystem.updateControllerList = this.updateControllerList.bind(
            this,
          );
        ...
        trackedControlsTickWrapper: function(time, delta) {
          if (this.el.components['motion-capture-replayer']) {
            return;
          }
        ...
        updateControllerList: function() {
          ...
          this.gamepads.forEach(function(gamepad) {
            if (trackedControlsSystem.controllers[gamepad.index]) {
              return;
            }
          ...
          for (i = 0; i < trackedControlsSystem.controllers.length; i++) {
            ...
            trackedControlsSystem.controllers[i] = {
              id: '___',
              index: -1,
              hand: 'finger',
            };
      ...
      AFRAME.registerSystem('recordingdb', {
        init: function() {
          ...
          request.onerror = function() {
            console.error(
              'Error opening IndexedDB for motion capture.',
              request.error,
            );
          };
          ...
          request.onupgradeneeded = function(evt) {
            ...
            objectStore.createIndex('recordingName', 'recordingName', {
              unique: true,
            });
